...
MICTF 是一种在线、无需训练数据的重定向方法，在精确接触建立*之后*维持 pad 级接触。它引入形态学导出的**接触模板**作为操作者意图与机器人执行之间的中间表示：每个模板将机器人手型模型映射为接触角色、局部 pad 坐标系，以及接触后的运动学约束。

框架的长期愿景是通过接触原语实现零样本跨具身遥操作。本页面反映当前进展，聚焦于位控型灵巧手与在线接触后维持。

## 为什么需要 MICTF

绝大多数重定向方法 —— DexPilot 式向量跟踪、关节复制、指尖 IK —— 回答的都是"指尖该去哪里？"它们在自由空间里表现良好。可一旦两个 pad 已经接触、操作者还在继续移动，这些方法都没有显式机制来维持接触面的持续贴合。结果就是接触在不知不觉中被撑开。

这并非实现问题，而是结构性的。一个仅被 3D 指尖向量约束的 4-DOF 机器人拇指，会留下一个未被约束的自由度 —— pad 绕"拇指-指尖轴"的旋转。DexPilot 的平滑正则项把这个自由度约束在初始解附近，但不提供任何接触几何引导。MICTF 转而用任务几何吸收这个自由度：一个 pad 法向对齐残差强制拇指 pad 朝向对侧表面。原本需要被正则化掉的"运动学冗余"，反而成了*维持*接触的机制。
...
| **接触模板** | 离散可执行对象（张开 / 精确捏合 / 多点捏合 / 闭合），携带接触角色、局部 pad 坐标系与求解器容差。 |
| **贝叶斯意图滤波** | 一个隐马尔可夫前向更新从 Quest 3/WebXR 观测中选择当前活跃模板，转移代价由机器人自身工作空间中的几何邻近度决定。 |
| **闭链投影** | 一个有界非线性最小二乘求解器在操作者持续移动的输入下维持所选接触；pad 法向残差通过接触几何消解拇指冗余。 |
...
5 只手的平均任务帧间隙：**MICTF 8.3–19.2 mm**，DexPilot 式 **27.7–76.6 mm**，关节复制 **23.1–55.7 mm**。所有基线均无法建立稳定接触（所有手型上 Acquire = 0.000）；MICTF 在每只手上都能过渡到保持接触（Acquire 0.222–0.778）。DexPilot 在向量误差上更低（33.9–47.6 mm，MICTF 为 54.7–106.8 mm），这是预期内的 —— MICTF 以端点向量保真度换取 pad 对齐。关闭闭链求解器后任务距离回落到基线水平，证实起作用的是投影层本身，而非仅仅是模板选择。
...
- **不是抓取规划器。** 维持小的 pad 间隙与坐标系对齐，本身并不构成力旋量闭合或稳定的物体操作。
